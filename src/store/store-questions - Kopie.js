 ...
 */
  importQuestion ({ dispatch }, payload) {
    ...
    if (questionImage) {
      const imageRegex = /([[(\s]?Раздаточный материал:([\s(\\n)])*)?(<раздатка>.*)?(\(pic:\s*)?[([]?https.+?\.(jpg|jpeg|png|gif)(.*<\/раздатка>)?[\s(\\n))\]]*/
      console.log('Image Regex: ' + imageRegex.match(question.question))
      question.question =
        question.question.replace(imageRegex, '@image@')
      const image = {
        id: payload.id,
